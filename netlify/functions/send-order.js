...
exports.handler = async (event) => {
  ...
  const safe = {
    ...
    iban: sanitize(iban || 'SK48 0900 0000 0052 4269 0350'),
  ...
  const discountRowHtml = safe.discountCode
    ? `<p style="margin:4px 0;color:#4caf50;font-weight:bold;">Zľava (${safe.discountCode} -${safe.discountPercent}%): -€${safe.discountAmount}</p>`
    : '';

  const discountTextLine = safe.discountCode
    ? `Zľava (${safe.discountCode} -${safe.discountPercent}%): -€${safe.discountAmount}\n`
    : '';
...
<body style="font-family:Arial,sans-serif;background:#f5f5f5;margin:0;padding:20px;">
  <div style="max-width:600px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.1);">
    ...
    <div style="padding:32px;">
      ...
      <div style="background:#f5f5f5;border-radius:10px;padding:16px;margin:16px 0;">
        ...
        ${discountRowHtml}
        <p style="margin:4px 0;color:#555;"><strong>Doprava (${safe.shippingMethod}):</strong> ${safe.shippingPrice === '0.00' ? '<span style="color:#4caf50;font-weight:bold;">ZADARMO ✅</span>' : '€' + safe.shippingPrice}</p>
        <p style="margin:12px 0 0;font-size:20px;font-weight:bold;color:#0d47a1;">CELKOM: €${safe.totalPrice}</p>
      </div>
  ...
  // EMAIL PRE ADMINA
  // ─────────────────────────────────────────────
  const adminDiscountBlock = safe.discountCode ? `
  ...
  <div style="max-width:600px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.1);">
    ...
    <div style="padding:32px;">
      ...
      <div style="background:#f5f5f5;border-radius:10px;padding:16px;margin:16px 0;">
        ...
        <p style="margin:12px 0 0;font-size:20px;font-weight:bold;color:#1b5e20;">CELKOM: €${safe.totalPrice}</p>
      </div>
